...
function business_one_page_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( '%s', 'post date', 'business-one-page' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( '%s', 'post author', 'business-one-page' ),
...
function business_one_page_categories( $blog = false ){
    ...
    if ( $categories_list && business_one_page_categorized_blog() ) {
        printf( '<span class="category">' . esc_html__( '%1$s', 'business-one-page' ) . '</span>', $categories_list ); // WPCS: XSS OK.
    }
}
endif;
