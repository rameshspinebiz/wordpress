...
?>
	...
	$wp_customize->add_control( new Scrollme_Customize_Info_Control( $wp_customize, 'scrollme_section_home', array(
			...
			'description' => __(sprintf('Use this ID to Create Scrolling Menu. <a target="_blank" href="%s">How to create Menu?</a>', 'http://accesspressthemes.com/documentation/scrollme/#!/scrollable_menu'), 'scrollme')
			)
		)
	);
    ...
    $wp_customize->add_section( 'scrollmepro_features', array( 'title' =>  __('ScrollMe Pro Features', 'scrollme'), 'priority' => '1000' ));
	$wp_customize->add_setting('theme_info_theme',array( 'default' => '', 'sanitize_callback' => 'sanitize_text_field' ));
	$wp_customize->add_control( new Theme_Info_Custom_Control( $wp_customize ,'theme_info_theme',array(
	    'label' => __( 'ScrollMe Pro Features' , 'scrollme' ),
	    'section' => 'scrollmepro_features',
   	)));
}
...
function scrollme_customize_scripts() {
	wp_enqueue_script( 'scrollme_custom_js', get_template_directory_uri() . '/inc/admin/js/custom.js', array( 'jquery' ), '20160410', true );
