	...
	<header class="page-header">
		<?php if ( have_posts() ) : ?>
			<h1 class="search-title">
				<?php /* translators: %s: search term */ printf( esc_html__( 'Search Results for: %s','tameer-construction'), '<span>' . esc_html( get_search_query() ) . '</span>' ); ?></h1>
		<?php else : ?>
			<h1 class="page-title"><?php esc_html_e( 'Nothing Found', 'tameer-construction' ); ?></h1>
		<?php endif; ?>
	</header>
	    ...
	    if($layout_setting == 'Left Sidebar'){ ?>
			...
			<div class="col-lg-8 col-md-8">
				...
				if ( have_posts() ) :
					...
					<p><?php esc_html_e( 'Sorry, but nothing matched your search terms. Please try again with some different keywords.', 'tameer-construction' ); ?></p>
					<?php
						get_search_form();
