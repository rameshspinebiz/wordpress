...
if ( ! function_exists( 'belise_setup' ) ) :
	...
	function belise_setup() {
		...
		add_theme_support( 'custom-background', apply_filters( 'belise_custom_background_args', array(
...
function belise_widgets_init() {
	...
	register_sidebar( array(
		...
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Add widgets here.', 'belise-lite' ),
...
function belise_scripts() {
	...
	if ( is_category() || is_search() || is_archive() || is_home() ) {
		wp_enqueue_script( 'masonry-call', get_template_directory_uri() . '/js/masonry-call.js', array( 'masonry' ), '20120206', true );
	}

	wp_enqueue_script( 'belise-functions', get_template_directory_uri() . '/js/functions.js', array(), '20120206', true );
}
...
/**
 * One Click Demo Import Filter function
 *
 * @return array
 */
function belise_import_files() {

	return array(
		array(
			'import_file_name'             => 'Belise Themeisle Demo Import',
			'local_import_file'            => BELISE_PHP_INCLUDE . '/demo-content/demo-content.xml',
			'local_import_widget_file'     => BELISE_PHP_INCLUDE . '/demo-content/widgets.wie',
			'local_import_customizer_file' => BELISE_PHP_INCLUDE . '/demo-content/belise-export.dat',
			'import_preview_image_url'     => trailingslashit( get_template_directory_uri() ) . 'screenshot.png',
		),
	);
}
add_filter( 'pt-ocdi/import_files', 'belise_import_files' );

/**
 * After one click demo import setup.
 */
function belise_after_demo_import() {
	/* Update menus after import */
	...
	$shop       = get_term_by( 'name', 'Shop Menu', 'nav_menu' );
	set_theme_mod( 'nav_menu_locations', array(
			'primary'      => $primary->term_id,
			'categories'   => $categories->term_id,
			'social-icons' => $social->term_id,
			'shop'         => $shop->term_id,
		)
	);
	...
	// $menu_categories = belise_get_food_sections();
}
add_action( 'pt-ocdi/after_import', 'belise_after_demo_import' );
		...
		<div class="comment-author-content">
			<div class="comment-author-title">
				...
				<div class="comment-meta commentmetadata"><a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ); ?>">
						<?php
						printf( '%1$s' , get_comment_date( 'd.m.Y' ) ); ?></a><?php edit_comment_link( esc_html__( '(Edit)' ,'belise-lite' ), '  ', '' );
					?>
				</div>
