...
if ( ! function_exists( 'belise_posted_on' ) ) :
	...
	function belise_posted_on() {
		...
		$time_string = sprintf( $time_string,
			...
			$byline = sprintf( esc_html_x( 'by %s', 'post author', 'belise-lite' ),
	...
	function belise_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list && belise_categorized_blog() ) { ?>
				<div class="cat-links <?php if ( ! class_exists( 'Jetpack' ) || ! Jetpack::is_module_active( 'sharedaddy' ) || ! has_action( 'belise_sharing_icons' ) ) { echo 'fullwidth'; } ?>">
				<?php echo '<span class="cat-links-title">' . esc_html__( 'Posted under', 'belise-lite' ) . '</span>'; //
				// WPCS: XSS OK.
			...
			<div class="author vcard" itemscope="itemscope" itemtype="http://schema.org/Person" itemprop="author">
			    <div class="container author-container">
					...
					<div class="author-content">
						<?php if ( ! empty( $author_name ) || ! empty( $author_job_title ) ) {
							echo '<div class="author-title">';
							if ( ! empty( $author_name ) ) { // Full name with link
							...
							if ( ! empty( $author_job_title ) ) { // Job title
						...
						<?php if ( ! empty( $author_description ) ) { // Description
							echo '<p class="author-description" itemprop="description">' . esc_html( $author_description ) . '</p>';
} ?>

						<?php if ( ! empty( $author_email ) || ! empty( $author_url ) || ! empty( $author_facebook ) || ! empty( $author_twitter ) || ! empty( $author_googleplus ) ) {
							echo '<div class="author-contact">';
							if ( ! empty( $author_email ) ) { // E-mail
								echo '<a href="mailto:' . esc_attr( $author_email ) . '"><i class="fa fa-envelope"></i></a>';
							}
							if ( ! empty( $author_url ) ) { // Website
								echo '<a href="' . esc_url( $author_url ) . '" target="_blank"><i class="fa fa-link"></i></a>';
							}
							if ( ! empty( $author_facebook ) ) { // Facebook
								echo '<a href="' . esc_url( $author_facebook ) . '" target="_blank"><i class="fa fa-facebook"></i></a>';
							}
							if ( ! empty( $author_twitter ) ) { // Twitter
								echo '<a href="https://twitter.com/' . esc_attr( $author_twitter ) . '" target="_blank"><i class="fa fa-twitter"></i></a>';
							}
							if ( ! empty( $author_googleplus ) ) { // Google+
								echo '<a href="' . esc_url( $author_googleplus ) . '?rel=author" rel="author" target="_blank"><i class="fa fa-google-plus"></i></a>';
							}
...
function belise_email_address() {
	if ( current_user_can( 'edit_theme_options' ) ) {
		$belise_contact_email = get_theme_mod( 'belise_contact_email', sprintf( '<a href="' . admin_url( 'customize.php?autofocus[control]=belise_contact_email' ) . '">%s</a>', esc_html__( 'Edit the email address in Customizer','belise-lite' )
		) );
	} else {
	...
	function belise_event_entry_footer() {
		if ( 'event' === get_post_type() ) {
			...
			?>
				...
				$categories_list = get_the_category_list( esc_html__( ', ', 'belise-lite' ) );
				if ( $categories_list && belise_categorized_blog() ) { ?>
				<div class="cat-links <?php if ( ! class_exists( 'Jetpack' ) || ! Jetpack::is_module_active( 'sharedaddy' ) ) { echo 'fullwidth'; } ?>">
					<?php echo '<span class="cat-links-title">' . esc_html__( 'Posted under', 'belise-lite' ) . '</span>'; //
					// WPCS: XSS OK.
					echo wp_kses_post( $categories_list );
					echo '</div>';
