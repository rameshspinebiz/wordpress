		...
		<div class="ribbon">
			<div class="container">
				<div class="row">
					...
					<?php if ( ! empty( $button_link ) && ! empty( $button ) ) :
						echo '<a href="' . esc_url( $button_link ) . '" class="btn">' . htmlspecialchars_decode( $button ) . '</a>';
					endif; ?>
				</div></div>
			</div>

	        <?php if ( ! empty( $image ) && ( $image != 'Upload Image' ) ) {
	...
	if ( ! empty( $belise_custom_media_id ) ) {
		...
		function form( $instance ) {
			...
			<p>
				...
				<input type="text" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" value="<?php if ( ! empty( $instance['title'] ) ) : echo esc_attr( $instance['title'] );
endif; ?>" class="widefat">
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'text' ) ); ?>"><?php esc_html_e( 'Text', 'belise-lite' ); ?></label><br/>
				<textarea class="widefat" rows="3" cols="20" name="<?php echo esc_attr( $this->get_field_name( 'text' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'text' ) ); ?>"><?php if ( ! empty( $instance['text'] ) ) : echo htmlspecialchars_decode( $instance['text'] );
endif; ?></textarea>
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'button_text' ) ); ?>"><?php esc_html_e( 'Button text', 'belise-lite' ); ?></label><br/>
				<input type="text" name="<?php echo esc_attr( $this->get_field_name( 'button_text' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'button_text' ) ); ?>" value="<?php if ( ! empty( $instance['button_text'] ) ) : echo esc_attr( $instance['button_text'] );
endif; ?>" class="widefat">
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'first-line' ) ); ?>"><?php esc_html_e( 'Button link', 'belise-lite' ); ?></label><br/>
				<input type="text" name="<?php echo esc_attr( $this->get_field_name( 'button_link' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'button_link' ) ); ?>" value="<?php if ( ! empty( $instance['button_link'] ) ) : echo esc_attr( $instance['button_link'] );
endif; ?>" class="widefat">
			</p>
			<p>
				...
				<input type="hidden" class="custom_media_display_in_customizer"
					   name="<?php if ( ! empty( $belise_image_in_customizer ) ) {
							echo esc_html( $belise_image_in_customizer );
} ?>"
					   value="<?php if ( ! empty( $instance['image_in_customizer'] ) ) : echo esc_attr( $instance['image_in_customizer'] );
endif; ?>">
				...
				<input type="text" class="widefat custom_media_url"
					   ...
					   value="<?php if ( ! empty( $instance['image_uri'] ) ) : echo esc_url( $instance['image_uri'] );
endif; ?>"
			...
			<input class="custom_media_id" id="<?php echo esc_attr( $this->get_field_id( 'custom_media_id' ) ); ?>"
				   ...
				   value="<?php if ( ! empty( $instance['custom_media_id'] ) ) : echo esc_attr( $instance['custom_media_id'] );
endif; ?>"/>
