 ...
 * @param null
 *
 * @return array $education_web_features_services_column_number
 *
 */
if ( ! function_exists( 'education_web_features_services_column_number' ) ) :
	function education_web_features_services_column_number() {
		$education_web_features_services_column_number = array(
			2 => esc_html__( '2', 'education-web' ),
			3 => esc_html__( '3', 'education-web' ),
			4 => esc_html__( '4', 'education-web' )
		);

		return apply_filters( 'education_web_features_services_column_number', $education_web_features_services_column_number );
	}
endif;
 ...
 * @since 1.0.0
 */
if ( ! class_exists( 'Education_web_featureservices' ) ) {

	class Education_web_featureservices extends WP_Widget {
		...
		function __construct() {
			...
			parent::__construct(
				...
				*/
				esc_html__( 'OT : Features Services', 'education-web' ),

				/**
				 * Widget description
				*/
				array( 'description' => esc_html__( 'A Widget Display Main Features Services Area', 'education-web' ), )
			);
		...
		*/
		...
		<?php if( $page_id != 0 ){ ?>
                <p>
                    ...
                    <br/>
                    <small><?php esc_html_e( 'Select Features Services Pages', 'education-web' ); ?></small>
					<?php
			...
			<?php } else{ ?>
                ...
                <label><?php esc_html_e( 'Select Features Services Pages', 'education-web' ); ?>:</label>
	                            ...
	                            <div class="repeater-table">
	                                ...
	                                <div class='sp-repeater-inside hidden'>
	                                    ...
	                                    <div class="sp-repeater-control-actions">
	                                        ...
	                                        <a class="button button-link sp-postid alignright" target="_blank" data-href="<?php echo esc_url( admin_url( 'post.php?post=POSTID&action=edit' ) ); ?>" href="<?php echo esc_url( admin_url( 'post.php?post='.$features['page_id'].'&action=edit' ) ); ?>"><?php esc_html_e('Full Edit','education-web'); ?></a>
	                                    </div>
	                                </div>
	                            </div>
							<?php
            ...
            <p>
                ...
                <select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'column_number' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'column_number' ) ); ?>">
					<?php
						$education_web_features_services_column_numbers = education_web_features_services_column_number();
						foreach ( $education_web_features_services_column_numbers as $key => $value ) {
					?>
		...
		?>
			<section class="features edu-widget">
				<div class="container">
					<div class="row">
						<?php
							$count = 1;
                        ...
                        if( !empty( $education_web_child_page_args ) ){
							...
							$servicespage = new WP_Query( $education_web_child_page_args );								
						...
						?>
							<div class="features-single <?php echo esc_attr( $init_column ) ?> features-<?php echo intval( $count ); ?>">
								<?php if( !empty( $icon ) ){ ?>
	...
	} // Class Education_web_featureservices ends here
}
 ...
 *
 */
if ( ! function_exists( 'Education_web_featureservices' ) ) :

	function Education_web_featureservices() {
		register_widget( 'Education_web_featureservices' );
	}

endif;
add_action( 'widgets_init', 'Education_web_featureservices' );